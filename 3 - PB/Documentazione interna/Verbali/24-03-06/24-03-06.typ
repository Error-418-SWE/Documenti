...
=== Database
Il gruppo ha deciso di scegliere la linea di progettazione che vede il database come parte del capitolato, e in particolare come data layer del software. Inoltre si è scelto di accedervi tramite backend, poiché farlo tramite browser porterebbe a numerosi problemi di sicurezza.
...
Il dubbio sull'implementarle o meno, poiché quest'ultime sono prive di comportamento specifico e risultano un semplice aggregato di attributi, è stato risolto grazie all'intervento del #cardin, che nel meeting ha spiegato al gruppo il significato di questo tipo di classi, chiamate _anemiche_, e ha asserito che la loro implementazione è sensata.
=== Business Logic
Il gruppo ha discusso sull'effettiva presenza della business logic in seguito al feedback ricevuto dal #cardin. La bl è stata individuata principalmente nella parte che interroga l’API REST esterna e negli oggetti che rappresentano le istanze del database. Si è discusso il modo in cui gli oggetti della bl devono essere convertiti in oggetti della presentation logic, e si è posta la norma di non utilizzare gli oggetti ritornati dalle API direttamente nella webapp.
