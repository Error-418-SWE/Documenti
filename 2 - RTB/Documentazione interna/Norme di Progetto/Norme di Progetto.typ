...
I meeting avranno cadenza variabile, e saranno fissati al termine di altri meeting oppure via e-mail. Il contenuto di ogni incontro sarà fissato all'interno del relativo verbale. Ognuno di questi verbali sarà validato dal proponente tramite l'apposizione di una firma, e sarà liberamente consultabile all'interno del repository GitHub del gruppo dedicato ai documenti (_#link("https://github.com/Error-418-SWE/Documenti/tree/main")_), al percorso `NomeBaseline/Documentazione esterna/Verbali`, dove `NomeBaseline` è uno tra:
- 1 - Candidatura;
- 2 - RTB;
- 3 - PB.

=== Documentazione prodotta
In questa sezione viene descritta la documentazione prodotta dal gruppo nel processo di fornitura, la quale sarà resa disponibile al proponente, _Sanmarco Informatica_, e ai committenti, prof. Tullio Vardanega e Prof. Riccardo Cardin.
...
==== Piano di Qualifica
Nel Piano di Qualifica vengono descritte le modalità adottate per garantire la qualità del prodotto sviluppato.

==== Glossario
Il documento denominato Glossario vengono elencati e definiti in modo preciso tutti i termini rilevanti utilizzati all'interno del progetto. È un documento estremamente importante per evitare situazioni di ambiguità, e garantire così una comunicazione più sicura ed efficace tra tutte le parti coinvolte.
